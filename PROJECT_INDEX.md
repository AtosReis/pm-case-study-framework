# PROJECT INDEX

## Canonical

1. docs/canonical/01_PROJECT_BRIEF.md

## Framework

1. docs/framework/01_PM_OPERATING_FRAMEWORK.md

## Case Study

1. docs/case_study/01_CASE_STUDY_SCENARIO.md

## Future Files

- docs/case_study/02_ACTORS_AND_SYSTEMS.md
- docs/case_study/03_USER_JOURNEY.md
- docs/case_study/04_PRODUCT_DECISIONS.md
- docs/case_study/05_USE_CASES.md
- docs/tickets/01_ENGINEERING_TICKETS.md
- docs/templates/01_USER_STORY_TEMPLATE.md
- docs/templates/02_ACCEPTANCE_CRITERIA_TEMPLATE.md
- docs/templates/03_ENGINEERING_HANDOFF_TEMPLATE.md

## Actors and Systems

1. docs/case_study/02_ACTORS_AND_SYSTEMS.md

## User Journey

1. docs/case_study/03_USER_JOURNEY.md

## Reusable PM Templates

1. docs/templates/01_USER_STORY_TEMPLATE.md
2. docs/templates/02_ACCEPTANCE_CRITERIA_TEMPLATE.md
3. docs/templates/03_ENGINEERING_HANDOFF_TEMPLATE.md

## PM Question Bank

1. docs/framework/02_PM_QUESTION_BANK.md

## Product Requirements Document

1. docs/case_study/06_PRD.md

## Workflow and Architecture

1. docs/case_study/07_WORKFLOW_AND_ARCHITECTURE.md

## Sample Newsletter Output

1. docs/case_study/08_SAMPLE_NEWSLETTER_OUTPUT.md

## Final

1. docs/final/01_EXECUTIVE_SUMMARY.md

## Interactive Generator

1. app/streamlit_app.py
2. requirements.txt
3. app/outputs/.gitkeep
